...
debugLog('Скрипт загружен. Bot name: ' + TELEGRAM_BOT_NAME);
...
function loadUserData(userId) {
    ...
    if (saved) {
        debugLog('Загружены данные для ' + userId);
        return JSON.parse(saved);
    } else {
        debugLog('Созданы новые данные для ' + userId);
        return {
            energy: 0,
            totalTaps: 0,
            power: 1,
...
function saveUserData() {
    ...
    debugLog('Сохранены данные для ' + currentUserId);
}
...
function initTelegramLogin() {
    debugLog('initTelegramLogin вызвана');
    let container = document.getElementById('telegram-login-container');
    if (!container) {
        debugLog('ОШИБКА: контейнер telegram-login-container не найден');
        return;
    }
    
    container.innerHTML = '';
    
    // Проверяем, что сайт открыт по HTTPS
    if (window.location.protocol !== 'https:') {
        debugLog('ПРЕДУПРЕЖДЕНИЕ: сайт открыт по ' + window.location.protocol + ', Telegram требует HTTPS');
    ...
    script.onerror = () => {
        debugLog('ОШИБКА: не удалось загрузить telegram-виджет');
        container.innerHTML = '<div style="color:#ff5555; text-align:center;">❌ Ошибка загрузки Telegram виджета. Проверьте интернет.</div>';
    };
    
    container.appendChild(script);
    debugLog('Telegram виджет добавлен. Bot name: ' + TELEGRAM_BOT_NAME);
}

window.onTelegramAuth = (user) => {
    debugLog('onTelegramAuth вызван: ' + JSON.stringify(user));
    if (user && user.id) {
        ...
        document.getElementById('userInfo').innerHTML = `✅ <strong>${data.userName}</strong><br><button id="logoutBtnTelegram" style="background:#ff4444;">Выйти</button>`;
        ...
        debugLog('Авторизация успешна для ' + user.id);
    } else {
        debugLog('Ошибка: user не получен или нет id');
...
// ========== ЗАПУСК ==========
window.onload = () => {
    debugLog('Страница загружена, onload');
    
    let lastUserId = localStorage.getItem('last_karp_user');
    if (lastUserId) {
        debugLog('Найден сохранённый пользователь: ' + lastUserId);
        currentUserId = lastUserId;
        data = loadUserData(lastUserId);
        updateUI();
        startTimers();
        document.getElementById('telegram-login-container').style.display = 'none';
        
        let userName = data.userName || 'Игрок';
        document.getElementById('userInfo').innerHTML = `✅ <strong>${userName}</strong><br><button id="logoutBtnAuto" style="background:#ff4444;">Выйти</button>`;
        document.getElementById('logoutBtn').style.display = 'block';
        
        const logoutBtn = document.getElementById('logoutBtnAuto');
        if (logoutBtn) {
            logoutBtn.onclick = () => {
                localStorage.removeItem('last_karp_user');
                location.reload();

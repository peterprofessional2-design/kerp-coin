...
// ========== ДАННЫЕ (ПРИВЯЗАНЫ К TELEGRAM ID) ==========
...
function loadUserData(userId) {
    ...
    if (saved) {
        ...
        return {
            ...
            soundEnabled: true,
            lastDaily: 0,
            // Отдельные таймеры для каждой покупки
            lastPowerBuy: 0,
            lastAutoBuy: 0,
            lastCritBuy: 0,
            lastTapUpgrade: 0,
...
function updateAllTimers() {
    ...
    // Магазин
    updateTimerUI('buyPower', getRemaining(data.lastPowerBuy), 'powerTimer');
    updateTimerUI('buyAuto', getRemaining(data.lastAutoBuy), 'autoTimer');
    updateTimerUI('buyCrit', getRemaining(data.lastCritBuy), 'critTimer');
    
    // Улучшения
    updateTimerUI('upTap', getRemaining(data.lastTapUpgrade), 'tapUpgradeTimer');
    updateTimerUI('upAuto', getRemaining(data.lastAutoUpgrade), 'autoUpgradeTimer');
    updateTimerUI('upCrit', getRemaining(data.lastCritUpgrade), 'critUpgradeTimer');
    updateTimerUI('upCore', getRemaining(data.lastEnergyUpgrade, 60), 'coreTimer');
}

function updateTimerUI(buttonId, remaining, timerSpanId) {
    ...
    if (remaining > 0) {
        ...
        if (buttonId === 'buyPower') btn.innerHTML = 'КУПИТЬ';
        else if (buttonId === 'buyAuto') btn.innerHTML = 'КУПИТЬ';
        else if (buttonId === 'buyCrit') btn.innerHTML = 'КУПИТЬ';
        else if (buttonId === 'upTap') btn.innerHTML = 'УЛУЧШИТЬ';
        else if (buttonId === 'upAuto') btn.innerHTML = 'УЛУЧШИТЬ';
        else if (buttonId === 'upCrit') btn.innerHTML = 'УЛУЧШИТЬ';
        else if (buttonId === 'upCore') btn.innerHTML = 'УЛУЧШИТЬ';
...
// ========== МАГАЗИН (с отдельными таймерами) ==========
...
// ========== УЛУЧШЕНИЯ (с отдельными таймерами) ==========
...
document.getElementById('prestigeBtn').onclick = () => {
    ...
    data.auto = 0;
    data.crit = 0;
    data.maxTapEnergy = 50;
    data.tapEnergy = 50;
    data.lastDaily = 0;
    data.lastPowerBuy = 0;
    data.lastAutoBuy = 0;
    data.lastCritBuy = 0;
    data.lastTapUpgrade = 0;
    data.lastAutoUpgrade = 0;
    data.lastCritUpgrade = 0;
    data.lastEnergyUpgrade = 0;
...
document.getElementById('hardResetBtn').onclick = () => {
    if (!currentUserId) { toast('⚠️ Войдите через Telegram'); return; }
    if (confirm('💀 ЖЁСТКИЙ СБРОС - удалить ВСЁ (включая престиж)?')) {
...
document.getElementById('tonSoonBtn').onclick = () => toast('⏳ TON кошелёк будет в следующем обновлении');
...
// Сохраняем lastUserId после входа
window.onTelegramAuth = (user) => {
    if (user && user.id) {
        ...
        document.getElementById('userInfo').innerHTML = `✅ <strong>${user.first_name}</strong><br><button id="logoutBtnTelegram" style="background:#ff4444;">Выйти</button>`;
        document.getElementById('logoutBtn').style.display = 'block';
        document.getElementById('logoutBtnTelegram').onclick = () => {
...
document.getElementById('logoutBtn').onclick = () => {
    if (confirm('Выйти из аккаунта? Прогресс останется сохранённым на этом устройстве.')) {
        localStorage.removeItem('last_karp_user');
        location.reload();
    }
};
...
// ========== ЗАПУСК ==========
window.onload = () => {
    let lastUserId = localStorage.getItem('last_karp_user');
    if (lastUserId) {
        ...
        startTimers();
        document.getElementById('telegram-login-container').style.display = 'none';
        document.getElementById('userInfo').innerHTML = `✅ Вы вошли как ID: ${currentUserId}<br><button id="logoutBtnAuto" style="background:#ff4444;">Выйти</button>`;
        document.getElementById('logoutBtn').style.display = 'block';

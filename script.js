...
function displayLeaderboard() {
    ...
    if (globalLeaderboard.length === 0) {
        container.innerHTML = "Нет игроков. Авторизуйтесь через Telegram!";
        return;
    }
    let html = "";
    globalLeaderboard.forEach((player, idx) => {
        ...
        html += `<div class="${cls}">${medal}#${idx+1} ${player.name.substring(0,20)} | 👆 ${player.taps.toLocaleString()} taps | 🪙 ${player.kerpBalance.toFixed(5)} KERP</div>`;
    });
...
function updatePlayerInfo() {
    ...
    if (data.userId) {
        ...
        div.innerHTML = `✅ Авторизован: <strong>${data.userName}</strong><br>🏆 Рейтинг: #${rank}<br>👆 Тапов: ${data.totalTaps.toLocaleString()}<br>🪙 KERP: ${data.kerpBalance.toFixed(5)}`;
    } else {
        div.innerHTML = `🔐 Не авторизован<br><span style="font-size:12px">Войдите через Telegram в настройках</span>`;
    }
}
...
function handleReferralOnLoad() {
    ...
    if (ref && !data.referredBy && !data.userId) {
        ...
        toast('🎁 Вас пригласили! Войдите через Telegram, чтобы получить 1000 KERP бонус!');
    }
}
...
document.getElementById("tapButton").onclick = () => {
    ...
    // Фиксированная награда за тап: 0.00001 KERP
...
// ========== АВТО-ДОХОД (даёт маленький KERP) ==========
setInterval(() => {
    ...
    if (data.tapEnergy > data.maxTapEnergy) data.tapEnergy = data.maxTapEnergy;
    if (data.auto > 0) {
        // Авто даёт 0.00001 за каждые 10 автоматических тапов
        let kerpAuto = KERP_PER_TAP * data.auto * data.prestigeMultiplier / 10;
        addKerp(kerpAuto);
    }
    updateUI();
    save();
...
// ========== ЕЖЕДНЕВНАЯ НАГРАДА (только энергия) ==========
...
document.getElementById("guestLoginBtn").onclick = () => {
    ...
    data.userId = "guest_" + Date.now() + "_" + Math.random().toString(36).substr(2, 6);
    data.userName = name.substring(0, 20);
